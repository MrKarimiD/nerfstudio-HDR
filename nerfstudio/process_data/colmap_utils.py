...
def colmap_to_json_for_different_Exposure(
    recon_dir: Path,
    output_dir: Path,
    mask1_path: Path = None,
    mask2_path: Path = None,
    e1_path: Path = None,
    e2_path: Path = None,
    ...
    for im_id, im_data in im_id_to_image.items():
        ...
        e1_name = name.replace('.png', '.exr')
        name = Path(e1_path.absolute() / e1_name)
        mask_name = im_data.name
        mask1_name = mask_name #.replace('.png', '.png')
        mask_path = Path(mask1_path.absolute() / mask1_name).as_posix()
        ...
        if image_rename_map is not None:
            name = image_rename_map[name]
        e2_name = name.replace('.png', '.exr')
        name = Path(e2_path.absolute() / e2_name)
        mask_name = im_data.name
        mask2_name = mask_name #.replace('.png', '.png')
        mask_path = Path(mask2_path.absolute() / mask2_name).as_posix()
